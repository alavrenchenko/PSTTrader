...
namespace ProSecuritiesTrading.MOEX.FIX.Base.Message.ASTS
{
    public class Heartbeat
    {
        ...
        static Heartbeat()
        {
            ...
            MsgTypeBytes[length + 1] = 48; // 0
        }

        public static HeartbeatData GetHeartbeatData(byte[] buffer, int index)
        {
            HeaderData headerData = new HeaderData();
            headerData.MsgType = "0";

            HeartbeatData heartbeatData = new HeartbeatData(buffer, headerData);
                    ...
                    case 49: // 1
                        {
                            ...
                            else if (buffer[index] == 49) // 1
                            {
                                ...
                                if (buffer[index] == 50) // 2 (TestReqID (112))
                                {
                                    ...
                                    valueBytes = Messages.GetValueBytes(buffer, index);
                                    heartbeatData.TestReqIDBytes = valueBytes;
                                    heartbeatData.TestReqID = Encoding.UTF8.GetString(valueBytes);
                                    index += valueBytes.Length;
                                }
                            }
                            else if (buffer[index] == 48) // 0 CheckSum (10)
                            {
                                ...
                                {
                                    heartbeatData.CheckSum = checkSumValue;
                                }
        ...
        public unsafe static byte[] UnsafeGetBytes(ProSecuritiesTrading.MOEX.FIX.Base.Group.Header header, int msgSeqNum, bool origSendingTime, byte[] testReqID)
        {
            ...
            int bodyLengthValue = MsgTypeBytes.Length + header.SenderAndTargetCompIDWithSOH.Length + msgSeqNumLength + ((header.PossDupFlag > 0) ? 5 : 0) + ((header.PossResend > 0) ? 5 : 0) + (sendingTimeValue.Length + 3) + ((origSendingTime == true) ? (sendingTimeValue.Length + 5) : 0) + ((testReqID != null) ? (testReqID.Length + 5) : 0) + 3;
