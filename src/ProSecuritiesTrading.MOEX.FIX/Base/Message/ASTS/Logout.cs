...
namespace ProSecuritiesTrading.MOEX.FIX.Base.Message.ASTS
{
    public class Logout
    {
        ...
        static Logout()
        {
            ...
            MsgTypeBytes[length + 1] = 53; // 5
        }

        public static LogoutData GetLogoutData(byte[] buffer, int index)
        {
            HeaderData headerData = new HeaderData();
            headerData.MsgType = "5";

            LogoutData logoutData = new LogoutData(buffer, headerData);
                    ...
                    case 53: // 5
                        {
                            ...
                            else if (buffer[index] == 56) // 8 (Text (58))
                            {
                                index += 2;
                                valueBytes = Messages.GetValueBytes(buffer, index);
                                logoutData.Text = Encoding.UTF8.GetString(valueBytes);
                                index += valueBytes.Length;
                            }
                    ...
                    case 49: // 1
                        {
                            ...
                            else if (buffer[index] == 48) // 0 CheckSum (10)
                            {
                                ...
                                {
                                    logoutData.CheckSum = checkSumValue;
                                }
